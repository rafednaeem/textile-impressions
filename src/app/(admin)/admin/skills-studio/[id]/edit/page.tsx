...
import type { WorkshopMedia } from "@/types/database"
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setVideos(
          ((w.workshop_media as WorkshopMedia[] | null) || [])
            .sort((a, b) => a.sort_order - b.sort_order)
            .map((m) => ({ url: m.url, media_type: m.media_type, storage_path: m.storage_path }))
        )
        setForm({
          title: w.title || "",
