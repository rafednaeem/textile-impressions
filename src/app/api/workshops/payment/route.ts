...
export async function POST(request: Request) {
  try {
    ...
    // Use service role to bypass RLS for guest-accessible operations
    ...
    const { data: workshop, error: wErr } = await serviceRole
      .from("workshops")
      .select("id, fee, title")
    ...
    const { data: existingPayment } = await serviceRole
